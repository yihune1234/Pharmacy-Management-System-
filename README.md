...
Configure local MySQL server (default in installer: host localhost, user root, empty password, DB pharmacy_db).
Run installer via browser or CLI-accessible PHP environment.
Installer creates schema, triggers/views, and default admin employee record.
...
database/populate_sample_data.php references columns/tables that do not fully match database/install.php schema (for example, role and medicine field differences, plus admin/emplogin usa[...]
...
database/populate_sample_data.php
