...
<body class="bg-[#f8fafc]">
    ...
    <div class="mb-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
            <h2 class="text-xs font-black text-purple-600 uppercase tracking-widest mb-1">Operational Matrix</h2>
            <h1 class="text-4xl font-extrabold text-slate-900 tracking-tight">Medicine Inventory</h1>
        ...
        <div class="flex items-center space-x-3">
            <div class="relative group hidden sm:block">
                ...
                <input type="text" id="tableSearch" placeholder="Filter inventory..." 
                       class="bg-white border border-slate-200 pl-11 pr-4 py-3 rounded-2xl text-sm font-bold text-slate-700 focus:ring-4 focus:ring-purple-500/10 focus:border-purple-500 focus:outline-none transition-all w-64 shadow-sm">
            </div>
            <a href="add.php" class="bg-slate-900 text-white px-8 py-3.5 rounded-2xl text-sm font-black shadow-xl shadow-slate-200 hover:bg-slate-800 transition-all flex items-center group">
                <i class="fas fa-plus-circle mr-3 transform group-hover:rotate-90 transition-transform"></i> Register Medicine
            </a>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                <thead class="bg-slate-50/50">
                    <tr>
                        <th class="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Asset ID</th>
                        <th class="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Medicine Name</th>
                        <th class="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Category</th>
                        <th class="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Stock Status</th>
                ...
                <tbody class="divide-y divide-slate-50">
                    <?php if ($medicines && $medicines->num_rows > 0): ?>
                            ...
                            <tr class="hover:bg-slate-50/80 transition-all group">
                                ...
                                <td class="px-10 py-6">
                                    <div class="flex flex-col">
                                        <span class="text-sm font-black text-slate-900 group-hover:text-purple-600 transition-colors"><?php echo htmlspecialchars($row["Med_Name"]); ?></span>
                                        <span class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Loc: <?php echo htmlspecialchars($row["Location_Rack"]); ?></span>
                                    </div>
                                </td>
                                <td class="px-10 py-6">
                                    <span class="px-4 py-1.5 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black tracking-widest uppercase"><?php echo htmlspecialchars($row["Category"]); ?></span>
                                </td>
                                <td class="px-10 py-6">
                                    <div class="flex items-center space-x-3">
                                        <span class="text-lg font-black text-slate-900"><?php echo $row["Med_Qty"]; ?></span>
                                ...
                                <td class="px-10 py-6">
                                    <span class="text-sm font-black text-slate-900 tracking-tight">Rs. <?php echo number_format($row["Med_Price"], 2); ?></span>
                                </td>
                                <td class="px-10 py-6">
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="6" class="px-10 py-20 text-center text-slate-400 font-bold italic">No asset synchronization found.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
