...
<body class="bg-slate-50">
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
        <div>
            ...
            <p class="text-slate-500 mt-1 font-medium">Search and manage patient loyalty profiles.</p>
        </div>
        ...
        <form method="post" class="flex items-center space-x-2 w-full md:w-auto">
            <div class="relative flex-grow md:w-80">
                ...
                <input type="text" name="valuetosearch" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search by name, phone..." 
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 border-b border-slate-100">
                        <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Patient Name</th>
                        <th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Phone</th>
                        <th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Membership</th>
                        <th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Loyalty Points</th>
                    ...
                    <?php if ($search_result && $search_result->num_rows > 0): ?>
                        <?php while($row = $search_result->fetch_assoc()): ?>
                            <tr class="hover:bg-blue-50/30 transition-colors group">
                                <td class="px-8 py-5">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">ID: #<?php echo str_pad($row["cid"], 4, '0', STR_PAD_LEFT); ?></p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-5">
                                    <p class="text-slate-900 font-black"><?php echo number_format($row["points"] ?? 0); ?></p>
                                </td>
                                <td class="px-6 py-5 text-right">
                                    <a href="../sales/pos1.php?c_id=<?php echo $row['cid']; ?>" class="text-blue-600 hover:text-blue-700 font-bold text-xs uppercase tracking-widest">
                                        New Bill
                                    </a>
                                </td>
                            </tr>
                        ...
                        <tr>
                            <td colspan="5" class="px-8 py-20 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-lg font-bold text-slate-900">No patients found</h3>
                                    <p class="text-slate-500 max-w-xs mx-auto mt-2">Try a different search term or register a new patient.</p>
                                </div>
                            </td>
                        </tr>
                    <?php endif; ?>
