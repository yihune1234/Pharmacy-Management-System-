...
<style type="text/tailwindcss">
    @layer components {
        ...
        .nav-link-active { @apply bg-emerald-600 text-white shadow-lg shadow-emerald-500/20 translate-x-1; }
    }
</style>
...
<div class="flex h-screen bg-[#f8fafc] overflow-hidden">
    <!-- Sidebar -->
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-72 bg-slate-950 border-r border-slate-800 transition-transform duration-300 transform -translate-x-full lg:translate-x-0">
        <div class="flex flex-col h-full">
            <!-- Brand Logo -->
            <div class="p-8">
                <div class="flex items-center space-x-3 group cursor-pointer" onclick="location.href='<?php echo $path; ?>dashboard.php'">
                    <div class="w-12 h-12 bg-emerald-600 rounded-2xl flex items-center justify-center shadow-xl shadow-emerald-500/20 transform group-hover:rotate-6 transition-transform">
                    ...
                    <div>
                        ...
                        <span class="text-[10px] font-bold text-emerald-400 uppercase tracking-widest">Medical Ops</span>
                    </div>
                </div>
            </div>
            ...
            <nav class="flex-1 px-4 space-y-8 overflow-y-auto">
                <div>
                    <p class="px-5 text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Laboratory</p>
                    <div class="space-y-1">
                        ...
                        <a href="<?php echo $path; ?>inventory/view.php" class="nav-link <?php echo (strpos($_SERVER['PHP_SELF'], 'inventory') !== false) ? 'nav-link-active' : ''; ?>">
                            <i class="fas fa-pills text-lg"></i>
                            <span class="text-sm">Inventory Tracking</span>
                        </a>
                    </div>
                </div>

                <div>
                    <p class="px-5 text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Public Safety</p>
                    <div class="space-y-1">
                        <a href="<?php echo $path; ?>customers/view.php" class="nav-link <?php echo (basename($_SERVER['PHP_SELF']) == 'view.php' && strpos($_SERVER['PHP_SELF'], 'customers') !== false) ? 'nav-link-active' : ''; ?>">
                            ...
                            <span class="text-sm">Patient Registry</span>
                        </a>
                        <a href="<?php echo $path; ?>customers/add.php" class="nav-link <?php echo (basename($_SERVER['PHP_SELF']) == 'add.php' && strpos($_SERVER['PHP_SELF'], 'customers') !== false) ? 'nav-link-active' : ''; ?>">
                            ...
                            <span class="text-sm">Enroll Member</span>
                        </a>
                    </div>
                </div>

                <div class="mt-8 px-4">
                    <a href="<?php echo $path; ?>sales/pos1.php" class="btn-primary btn-emerald w-full italic">
                        <i class="fas fa-prescription mr-3"></i> Process Sale
                    </a>
                </div>
            </nav>
    ...
    <div class="flex-1 lg:ml-72 flex flex-col min-h-screen transition-all duration-300">
        ...
        <header class="h-20 bg-white/70 glass-effect border-b border-slate-200/60 flex items-center justify-between px-8 sticky top-0 z-40">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h2 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-0.5">Clinical Protocol</h2>
                    <h1 class="text-lg font-bold text-slate-800 tracking-tight leading-none italic">Secure Session</h1>
                </div>
            </div>

            <div class="flex items-center space-x-2">
                <div class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                <span class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Medical Gateway Online</span>
            </div>
        </header>
